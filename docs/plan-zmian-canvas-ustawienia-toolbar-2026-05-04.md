...
Opcja `Środek krawędzi` przestaje być potrzebna jako przełącznik zachowania i powinna zostać usunięta z:
...
- wycinki mają midpointy,
- badge nie zasłaniają midpointów,
- midpoint outline przesuwa całą krawędź,
- midpoint wycinka dodaje nowy punkt,
- opcja `Środek krawędzi` znika z ustawień,
- drag bocznej krawędzi nie generuje niechcianego dryfu po osi `Y`.

## 5. Badge długości ma skalować cały kształt proporcjonalnie

### Objaw

Na czwartym obrazie użytkownik wskazał, że zmiana długości przez badge nie może już działać jak lokalna edycja jednej krawędzi.

### Wymaganie

Po wpisaniu nowej długości przez badge ma nastąpić proporcjonalne przeskalowanie całego kształtu, a nie rozciągnięcie tylko jednej ściany.

### Reguła docelowa

- zmiana jednej wartości długości przez badge wyznacza współczynnik skali,
- cały outline skaluje się jednolicie tym współczynnikiem,
- wszystkie powiązane wycinki również skalują się tym samym współczynnikiem,
- kształt ma zachować proporcje.

### Decyzja doprecyzowana

Skalowanie ma odbywać się względem punktu `x:0, y:0`.

To oznacza:

- pivot skalowania to punkt `0,0` geometrii gotowego kształtu,
- po skalowaniu lewa dolna baza referencyjna nadal ma mieć `x:0, y:0`,
- nie przesuwamy kształtu po operacji w sposób zmieniający tę bazę,
- wycinki skalują się zawsze tym samym współczynnikiem co outline.

### Moduły do ruszenia

- `ui/drawing_canvas.py`
- potencjalnie pomocnicza geometria w `core/geometry.py`
- testy:
  - `tests/test_drawing_canvas.py`
  - ewentualnie `tests/test_models_and_state.py`
