...
| `stage-2-pr4-002` | `open` | `stage-2` | `#4` | `review_comment:3137913119` | `ui/main_window.py` | `555` | Triangle dialog still calls `make_triangle()` outside the normal `_edit`/warning path, so invalid inputs can raise uncaught `ValueError` and leave invalid dialog values in in-memory config. | Still reproducible in `MainWindow._dlg_trojkat()`; worth fixing together with the freehand/UI geometry cleanup. |
| `stage-3-pr5-001` | `open` | `stage-3` | `#5` | `review_comment:3138030419` | `ui/main_window.py` | `61` | Startup uses `_workspace.sync()` directly, so initial canvases do not get `outline_edit_committed` / `outline_edit_rejected` connections until a later `_refresh_canvas()` call. | Still reproducible in `MainWindow.__init__()` on `feat/stage6`; first outline drag after opening an existing project can be dropped. |
| `stage-4-pr6-001` | `open` | `stage-4` | `#6` | `review_comment:3138202911` | `ui/main_window.py` | `125` | The `Wycinki` menu no longer exposes the existing cutout move workflow, leaving `_dlg_move_hole()` unreachable from the UI. | Still reproducible on `feat/stage6`; user-facing regression and likely safe to restore in a later UI-focused stage. |
| `stage-5-pr7-001` | `open` | `stage-5` | `#7` | `review_comment:3138630061` | `ui/dialogs/material_dialog.py` | `75` | `BlachyDialog` has only a `Close` button wired to reject, so catalog edits are silently discarded because `MainWindow._dlg_blachy()` only applies changes on `Accepted`. | Still reproducible on `feat/stage6`; high-priority functional regression in material catalog editing. |
| `stage-5-pr7-002` | `open` | `stage-5` | `#7` | `review_comment:3138630280` | `core/models.py` | `163` | `MaterialDefinition.from_dict()` / `to_dict()` still omit the new `min_sheet_length_cm` key from the dual-key migration pattern. | Still reproducible on `feat/stage6`; external/new-style payloads can silently lose `min_sheet_length_cm`. |
