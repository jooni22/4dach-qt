...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `stage-2-pr3-001` | `open` | `stage-2` | `#3` | `review_comment:3137012060` | `ui/main_window.py` | `393` | Starting draw mode creates and persists an empty plane before the user confirms the freehand polygon, which leaves a stray saved tab when drawing is cancelled. | Functional regression flagged by Codex review. |
| `stage-2-pr3-002` | `open` | `stage-2` | `#3` | `review_comment:3137018786` | `ui/main_window.py` | `396` | Re-entering draw mode can accumulate `polygon_closed` signal connections and create duplicate planes when the polygon is finally closed. | Functional regression flagged by Devin review. |
| `stage-2-pr3-003` | `open` | `stage-2` | `#3` | `review_comment:3137018682` | `core/project_state.py` | `289` | `move_hole_in_plane` is missing the new `plane.outline is None` guard, so geometry validation can crash on empty planes. | Functional regression flagged by Devin review. |
| `stage-2-pr3-004` | `open` | `stage-2` | `#3` | `review_comment:3137009525` | `ui/main_window.py` | `57` | Remove the redundant early assignment of `self._workspace.primary_canvas` before `self._workspace.sync()` populates it. | Safe local cleanup from Gemini review. |
| `stage-2-pr3-005` | `open` | `stage-2` | `#3` | `review_comment:3137009522` | `core/project_state.py` | `206` | Consider extracting repeated active-plane and outline validation into a helper to reduce duplicated guards. | Refactor suggestion from Gemini review; lower priority than regressions above. |
...
| Backlog ID | Status | Stage | Source PR | Source Ref | File | Line | Summary | Resolution / Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| _none yet_ | - | - | - | - | - | - | No deferred, rejected, fixed, or stale items have been recorded yet. | Update this table instead of deleting history later. |
